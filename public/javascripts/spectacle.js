$(function () {
  ...
  if ($sidebar.length) {
    ...
    var $drawer = $drawerLayout.find('.drawer')
    ...
    $drawerLayout.find('[data-drawer-slide]').click(function (e) {
      var $this = $(this)
      var direction = $this.data('drawer-slide')
...
Traverse.prototype.calcPoints = function () {
  ...
  this.$targets.each(function () {
    var $tar = $(this)
    var pt = $tar.offset().top // Math.round($tar.offset().top - _this.options.threshold);
    $tar.targetPoint = pt
...
Traverse.prototype._events = function () {
  ...
  this.$element.on('click', 'a[href^="#"]', function (e) { // 'click.zf.traverse'
    ...
    var scrollPos = $(arrival).offset().top - _this.options.barOffset // - _this.options.threshold / 2 - _this.options.barOffset;
...
Traverse.prototype._updateActive = function () {
  ...
  if (winPos + this.winHeight === this.docHeight) { curIdx = this.points.length - 1 } else if (winPos < this.points[0]) { curIdx = 0 } else {
    ...
    var _this = this
    var curVisible = this.points.filter(function (p, i) {
      return isDown
